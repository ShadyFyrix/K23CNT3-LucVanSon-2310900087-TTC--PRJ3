...
public interface LvsReviewRepository extends JpaRepository<LvsReview, Long> {
        ...
        // ==================== EAGER LOADING QUERIES FOR ADMIN ====================
        // These queries use JOIN FETCH to eagerly load relationships and prevent
        // LazyInitializationException

        /**
         * Find review by ID with eager loading of user and project
         * Prevents LazyInitializationException in admin templates
         */
        @Query("SELECT r FROM LvsReview r " +
                        "LEFT JOIN FETCH r.lvsUser " +
                        "LEFT JOIN FETCH r.lvsProject " +
                        "WHERE r.lvsReviewId = :reviewId")
        Optional<LvsReview> findByIdWithUserAndProject(@Param("reviewId") Long reviewId);

        /**
         * Find all reviews with eager loading for admin list page
         */
        @Query(value = "SELECT r FROM LvsReview r " +
                        "LEFT JOIN FETCH r.lvsUser " +
                        "LEFT JOIN FETCH r.lvsProject", countQuery = "SELECT COUNT(r) FROM LvsReview r")
        Page<LvsReview> findAllWithUserAndProject(Pageable pageable);

        /**
         * Find reviews by approval status with eager loading
         */
        @Query(value = "SELECT r FROM LvsReview r " +
                        "LEFT JOIN FETCH r.lvsUser " +
                        "LEFT JOIN FETCH r.lvsProject " +
                        "WHERE r.lvsIsApproved = :isApproved", countQuery = "SELECT COUNT(r) FROM LvsReview r WHERE r.lvsIsApproved = :isApproved")
        Page<LvsReview> findByLvsIsApprovedWithUserAndProject(@Param("isApproved") Boolean isApproved,
                        Pageable pageable);

        /**
         * Find reviews by project with eager loading
         */
        @Query(value = "SELECT r FROM LvsReview r " +
                        "LEFT JOIN FETCH r.lvsUser " +
                        "LEFT JOIN FETCH r.lvsProject p " +
                        "WHERE p.lvsProjectId = :projectId", countQuery = "SELECT COUNT(r) FROM LvsReview r WHERE r.lvsProject.lvsProjectId = :projectId")
        Page<LvsReview> findByProjectIdWithUserAndProject(@Param("projectId") Long projectId, Pageable pageable);

        /**
         * Find APPROVED reviews by project with eager loading (for user-facing pages)
         */
        @Query(value = "SELECT r FROM LvsReview r " +
                        "LEFT JOIN FETCH r.lvsUser " +
                        "LEFT JOIN FETCH r.lvsProject p " +
                        "WHERE p.lvsProjectId = :projectId AND r.lvsIsApproved = :isApproved", countQuery = "SELECT COUNT(r) FROM LvsReview r "
                                        +
                                        "WHERE r.lvsProject.lvsProjectId = :projectId AND r.lvsIsApproved = :isApproved")
        Page<LvsReview> findByProjectIdAndApproved(
